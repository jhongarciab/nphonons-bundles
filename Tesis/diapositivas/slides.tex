...
\begin{frame}{Motivación}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.52\textwidth}
			...
			\vspace{8pt}
			{\small
				Sin embargo, obtener estados de $n$ fotones de forma
				\textbf{determinista y con alta pureza} es técnicamente
				difícil: los métodos actuales son probabilísticos o
				requieren post-selección con eficiencia exponencialmente
				decreciente en $n$.
			}
		\end{column}
...
\begin{frame}{Antecedentes: emisión de paquetes de fotones}
	
	\begin{columns}[T]
		
		% ── Columna izquierda: Muñoz 2014 ────────────────────────────
		\begin{column}{0.48\textwidth}
			\textbf{Emitters of $N$-photon bundles}\\
			{\footnotesize Muñoz et al., \textit{Nat. Photonics} \textbf{8}, 550 (2014) \cite{munoz2014}}
			
			\vspace{8pt}
			{\small
				\begin{itemize}
					\item \textbf{Qué:} fuente continua de paquetes de $N$
					fotones con $N$ sintonizable por la frecuencia
					del láser.
					\item \textbf{Cómo:} un sistema de dos niveles en una
					cavidad excitado coherentemente en régimen
					dispersivo; la disipación de Purcell induce la
					emisión en grupos de $N$ fotones correlacionados.
					\item \textbf{Resultado:} $\pi_2 \approx 98.8\%$
					en condiciones óptimas; ${\sim}85\%$ en
					semiconductores y ${\sim}90\%$ de tres fotones
					en circuit QED. Introduce la pureza $\pi_N$ y
					las funciones $g_N^{(n)}$ como observables
					de paquetes.
				\end{itemize}
			}
		\end{column}
		...
		\hfill
		% ── Columna derecha: Muñoz 2018 ───────────────────────────────
		\begin{column}{0.48\textwidth}
			\textbf{Filtering multiphoton emission from state-of-the-art cavity QED}\\
			{\footnotesize Muñoz et al., \textit{Optica} \textbf{5}, 14 (2018) \cite{munoz2018}}
			
			\vspace{8pt}
			{\small
				\begin{itemize}
					\item \textbf{Qué:} mejora de la pureza de emisión
					multifonónica mediante filtrado espectral.
					\item \textbf{Cómo:} los fotones espurios y los paquetes
					de $N$ fotones se emiten en ventanas espectrales
					distintas; el filtrado en frecuencia suprime
					la contaminación de fotón único. Expresiones
					analíticas para tasas, pureza y distribución
					espectral.
					\item \textbf{Resultado:} el filtrado relaja el
					requisito de acoplamiento fuerte
					($g > \gamma_{a,\sigma}$) al régimen
					\textit{bad-cavity} con cooperatividad $C > 1$,
					abriendo implementaciones en semiconductores
...
\begin{frame}{Estadística de emisión: $g^{(n)}(0)$}
			...
			\begin{tikzpicture}[remember picture]
				\node[inner sep=0] (zoomfig) {%
					\resizebox{0.88\textwidth}{!}{%
						\input{../../codes/results/oficial/pgf/g3_dip_zoom.pgf}%
					}%
				};
			\end{tikzpicture}
			\hfill\null
				...
				\begin{column}{0.48\textwidth}
					...
					\begin{tikzpicture}[remember picture]
						...
						\begin{scope}[shift={(stackedfig.south west)},
							...
							\coordinate (dipUL) at (0.60,0.47);
							\coordinate (dipLR) at (0.78,0.39);
							\coordinate (dipLL) at (0.60,0.39);
							\draw[gray!50, line width=0.35pt] (dipUL) rectangle (dipLR);
						\end{scope}
					\end{tikzpicture}\\[2pt]
				\end{column}
	...
	\begin{tikzpicture}[remember picture,overlay]
		\draw[gray!70, line width=0.35pt]
			(dipUL) -- ([yshift=7mm]zoomfig.east);
		\draw[gray!70, line width=0.35pt]
			(dipLL) -- ([yshift=-7mm]zoomfig.east);
	\end{tikzpicture}
...
\begin{frame}{Trayectorias cuánticas de Monte Carlo}
	...
	\begin{columns}[T]
		\begin{column}{0.55\textwidth}
			{\small
				Sistema en $|vv,0\rangle$ el $97\%$ del tiempo.
				Las oscilaciones super-Rabi construyen población en
				$|\Psi_+,2\rangle$ hasta $\sim 4\%$.
				Dos clicks en ventana $\Delta\tau \approx 169\,\omega_b^{-1}
				\approx 0.08\,\kappa^{-1}$: emisión en cascada de dos fonones
				fuertemente correlacionados.
			}
		\end{column}
		\begin{column}{0.42\textwidth}
			{\small
				Tras la emisión, fotón óptico en
				$|\Psi_+,0\rangle \to |vv,0\rangle$ anuncia el evento
				$\Rightarrow$ \textbf{Señal de heraldo}.\\
				$|\Psi_-\rangle$ desocupado ($P_{\Psi_-}^{\max} \approx 9\times10^{-3}$):
				dinámica confinada al subespacio brillante.
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Pureza de emisión $\Pi_n$}
	...
	{\small\textbf{Problema:} $g^{(n)}(0)$ no cuantifica la fracción
		de emisión en paquetes de exactamente $n$ fonones
		$\Rightarrow$ Muñoz et al.\ \cite{munoz2014,munoz2018}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.48\textwidth}
			{\small
				$\Pi_n$ y $g^{(n)}(0)$ son \textbf{anticorrelados}:
				el máximo de $g^{(n)}(0)$ no coincide con
				el máximo de $\Pi_n$ \cite{munoz2014}.
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Mapas de pureza $\Pi_n$}
	...
	{\small
		Región de alta pureza se estrecha con $n$:
		$\Omega_\mathrm{eff}^{(n)} \propto (\lambda/\omega_b)^n$
		exige mayor $\lambda$ para órdenes superiores.\\[6pt]
...
\begin{frame}{Factibilidad experimental}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.42\textwidth}
			\vspace{4pt}
			\textbf{Sistema de referencia}\\[4pt]
			{\small
				2 QDs en nanocavidad acústica de THz
				\cite{weig2004,rozas2009}:\\[6pt]
				\begin{tabular}{ll}
	...
	\textbf{Resultado principal}\\[4pt]
	{\small
		\begin{center}
			\begin{tabular}{lll}
			& 1QD \cite{bin2020} & 2QD \\[3pt]
			$\Pi_3 > 95\%$ & $Q \sim 500$ & $Q \sim 90$
			\end{tabular}
		\end{center}
		
		\vspace{6pt}
		Origen: factor $\sqrt{2}$ superradiante acelera la cascada,
		reduciendo la contaminación por procesos fuera de resonancia.
	}
...
\begin{frame}{Conclusiones}
	...
	\begin{itemize}\setlength{\itemsep}{8pt}
		...
		\item \textbf{Perspectivas:} extender a $N > 2$ QDs; estudiar el
		impacto de asimetría entre emisores; incorporar $\gamma_\varphi$
		mediante trayectorias cuánticas exactas de Monte Carlo.
