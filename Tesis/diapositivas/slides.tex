...
\begin{frame}{Ecuación maestra de Lindblad}
	...
	La disipación se incorpora mediante la ecuación:
	\begin{equation*}
	...
	La cavidad es el \textbf{canal principal de emisión}.
...
\begin{frame}{Conclusiones}
	...
	\begin{itemize}\setlength{\itemsep}{8pt}
		...
		\item El factor  $\sqrt{2}$ y el corrimiento $-J$ en las
		...
		$Q \sim 500$ requeridos por el 1QD \cite{bin2020}. 
		...
		\item El fotón heraldo habilita protocolos de información cuántica
		fonónica.
...
\begin{frame}{Implementación experimental: fonones SAW en chip}
	
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			\hspace{15pt}
			\includegraphics[width=0.70\textwidth]{img/fig_gustafsson}
		\end{column}
		\begin{column}{0.48\textwidth}
			\vspace{25pt}
			¿Por qué fonones SAW en lugar de fotones de microondas? \cite{gustafsson2014}
			\vspace{6pt}
			\begin{itemize}
				\item A $5\,\text{GHz}$: $\lambda_\text{fotón} \sim 6\,\text{cm}$,\;
				$\lambda_\text{fonón} \sim 600\,\text{nm}$
				\item Infraestructura acústica de la \emph{misma escala} que el qubit
				\item Sin \textit{crosstalk} electromagnético entre líneas vecinas
				\item Retardo de tránsito $\sim 33\,\text{ns}$ sobre $100\,\mu\text{m}$: 
				controlable con puertas cuánticas de $\sim 1\,\text{ns}$
			\end{itemize}
		\end{column}
	\end{columns}
