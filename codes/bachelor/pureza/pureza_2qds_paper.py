...
if not RERUN:
    ...
    # GUARDAR DATOS
    # -----------------------------------------------------------------------------
    np.savez(RESULTS_DATA_DIR / 'pureza_2qd_data.npz',
    ...
    data = np.load(RESULTS_DATA_DIR / 'pureza_2qd_data.npz')
...
fig, axes = plt.subplots(1, n_cols, figsize=(5.00, 3.00), sharey=True)
if n_cols == 1:
    axes = [axes]
...
for col, n_b in enumerate(n_bundle_list):
    ...
    if col == 1:
        ax.set_xlabel(r'$\lambda/\omega_b$', fontsize=9)
    else:
        ax.set_xlabel('')
    ...
    if col == 0:
        ax.set_ylabel(r'$\kappa/\omega_b$', fontsize=9)
    else:
...
plt.savefig(RESULTS_OFICIAL_DIR / "purity_2qd_heatmap_paper.pdf", bbox_inches='tight')
plt.savefig(RESULTS_PGF_DIR / "purity_2qd_heatmap_paper.pgf")
...
ax_a.set_xlabel(r'$\kappa/\omega_b$', fontsize=9)
ax_a.set_ylabel(r'$\Pi_n$', fontsize=9)
...
ax_b.set_xlabel(r'$\lambda/\omega_b$', fontsize=9)
...
plt.savefig(RESULTS_OFICIAL_DIR / "purity_2qd_cuts_paper.pdf", bbox_inches='tight')
plt.savefig(RESULTS_PGF_DIR / "purity_2qd_cuts_paper.pgf")
