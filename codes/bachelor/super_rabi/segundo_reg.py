...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "pruebas"
...
ax.text(1e4, 0.75, r"$P_{0v}$", color="black")
ax.text(1e4, 0.20, r"$P_{2c}$", color="red")
...
axins.text(1e5, 0.75, r"$P_{0v}$", color="black", fontsize=11)
axins.text(1e5, 0.20, r"$P_{3c}$", color="green", fontsize=11)

plt.tight_layout()
plt.savefig(RESULTS_DIR / "rabi_segundo_reg.pdf", bbox_inches="tight")
plt.show()
