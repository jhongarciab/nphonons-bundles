...
CODES_DIR = Path(__file__).resolve().parents[2]
...
if not RERUN:
    ...
    np.savez(RESULTS_DATA_DIR / 'trayectoria_2qd_data.npz', **export)
    print("✓ Datos exportados: trayectoria_2qd_data.npz")

else:
    d      = np.load(RESULTS_DATA_DIR / 'trayectoria_2qd_data.npz', allow_pickle=True)
    clicks = d['clicks']
...
fig, axes = plt.subplots(3, 1, figsize=(6.60, 4.00), sharex=True)
...
axes[0].grid(False)
axes[0].text(400, 0.78, r'$P_{0vv}$', color='black', fontsize=12)
axes[0].text(400, 0.12, r'$P_{0\Psi_+}$', color='red', fontsize=12)
...
axes[1].set_ylabel("Poblaciones", fontsize=12, labelpad=10)
axes[1].grid(False)
axes[1].text(400, 2e-1, r'$P_{1\Psi_+}$', color='#c4942a', fontsize=12)
axes[1].text(400, 0.4e-7, r'$P_{1vv}$', color='black', fontsize=12)
...
axes[2].grid(False)
axes[2].text(380, 6e-2, r'$P_{2\Psi_+}$', color='#1f77b4', fontsize=12)
axes[2].text(380, 2e-7, r'$P_{2vv}$', color='black', fontsize=12)
...
plt.savefig(RESULTS_OFICIAL_DIR / "trayectoria_2qd.pdf", bbox_inches="tight")
plt.savefig(RESULTS_PGF_DIR / "trayectoria_2qd.pgf")
plt.close()
