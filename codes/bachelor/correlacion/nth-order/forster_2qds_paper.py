...
Figura paper con n = 2, 3 en función de Δ/ω_b, reutilizando la data guardada.
"""
...
# -----------------------------------------------------------------------------
RERUN = True # True para cargar datos guardados y no recalcular nada
...
if not RERUN:
    ...
    np.savez(
        RESULTS_DATA_DIR / "g_n_orders_stacked_paper_data.npz",
    ...
    data = np.load(RESULTS_DATA_DIR / "g_n_orders_stacked_data.npz")
...
# -----------------------------------------------------------------------------
fig, axes = plt.subplots(2, 1, figsize=(4.20, 2.50), sharex=True)

color_by_order = {2: "blue", 3: "green"}
...
for idx, (ax, n_order) in enumerate(zip(axes, n_orders[::-1])):
    ...
    ax.set_ylabel(rf"$g^{{({n_order})}}(0)$", fontsize=9)
...
axes[-1].set_xlabel(r"$\Delta/\omega_b$", fontsize=9)
...
#plt.show()
plt.savefig(RESULTS_OFICIAL_DIR / "g_n_orders_stacked_paper.pdf", bbox_inches="tight")
plt.savefig(RESULTS_PGF_DIR / "g_n_orders_stacked_paper.pgf")
plt.close()
