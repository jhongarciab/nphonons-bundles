...
Figura combinada: dos heatmaps g^(3) apilados verticalmente con eje x compartido.
  Panel (b) — barrido en lambda/omega_b
  Panel (c) — barrido en Omega/omega_b
...
if not RERUN:
    ...
    np.savez(
        RESULTS_DATA_DIR / "corr_2qds_combined_heatmap_data.npz",
        Delta_arr=Delta_arr,
    ...
    data = np.load(RESULTS_DATA_DIR / "corr_2qds_combined_heatmap_data.npz")
    Delta_arr = data["Delta_arr"]
    lambda_arr = data["lambda_arr"]
    Omega_arr = data["Omega_arr"]
    g3_map_b = data["g3_map_b"]
    g3_map_c = data["g3_map_c"]
...
ax_b.set_rasterization_zorder(0)
...
# SALIDA
# -----------------------------------------------------------------------------
plt.savefig(RESULTS_OFICIAL_DIR / "corr_2qds_combined_heatmap.pdf", bbox_inches="tight")
plt.savefig(RESULTS_PGF_DIR / "corr_2qds_combined_heatmap.pgf")
plt.close()
