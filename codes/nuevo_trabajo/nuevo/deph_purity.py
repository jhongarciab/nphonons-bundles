...
if not RERUN:
    ...
    # EXPORTAR
    # -------------------------------------------------------------------------
    np.savez(RESULTS_DATA_DIR / 'dephasing_vs_purity_data.npz',
             gphi_arr=gphi_arr,
    ...
    data = np.load(RESULTS_DATA_DIR / 'dephasing_vs_purity_data.npz')
...
fig.savefig(RESULTS_OFICIAL_DIR / "deph_purity.pdf", bbox_inches='tight')
fig.savefig(RESULTS_PGF_DIR / "deph_purity.pgf")
