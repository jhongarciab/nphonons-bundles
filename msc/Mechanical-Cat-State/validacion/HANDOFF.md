...
- Mecánico en marco rotante a ω_r: término estático δ_m b†b con
  δ_m = ω_m − ω_r.
- Qubit en marco rotante a 2ω_r: término estático (Δ_q/2)σ_z con
  Δ_q = ω_q − 2ω_r.
- Acoplamientos:
  `g_x (σ_+ e^{+2iω_r t} + σ_- e^{-2iω_r t})(b e^{-iω_r t} + b† e^{+iω_r t})`
  y `g_z σ_z (b e^{-iω_r t} + b† e^{+iω_r t})`.
- Drive: `ε(σ_+ + σ_-) + ε(σ_+ e^{+4iω_r t} + σ_- e^{-4iω_r t})`
  (conserva el término contrarrotante).
- Propagador sobre `T_r = 2π/ω_r`, con **ω_r = ω_m** (valor base, el
  mismo usado en todas las tareas anteriores).

**Verificación obligatoria antes de seguir**: con δ_m=0 y Δ_q=0, debe
reproducir los autovalores de la Tarea 18(ii) (ver
`tarea18_resultados.npz`/`tarea18_resultados.md` para los valores de
referencia: 6 tasas lentas, la más lenta λ≈4.82e-4, luego 6.91e-4,
9.91e-2, 0.158, 0.158, 0.161). Diferencia relativa < 1e-6 en esas 6
tasas. **Si no pasa esta verificación, no seguir** — hay un error en la
reconstrucción del Hamiltoniano en el nuevo marco.

Luego, en Γ₂/κ=0.52 y |α|²=3, barrer una rejilla (δ_m, Δ_q) de 9×9
alrededor de (δ_1, 0) [δ_1 es el mismo de siempre,
`δ_1 = g_x²(Im S_{1-} + Im S_{1+})`, calculado con el g_x de ese caso],
con rangos ±3|δ_1| en cada eje. Para cada punto de la rejilla (81 puntos
totales — **usar el patrón worker, un proceso por celda**) reportar
γ_bf, Im(λ_bf), γ_pf y la brecha de confinamiento. Ubicar el punto de
"resonancia vestida" (mínimo de γ_bf, que debe coincidir con
Im(λ_bf)≈0) y compararlo con (δ_1, 0) — la hipótesis es que la
resonancia vestida real está desplazada de (δ_1, 0).
...
(a) γ_bf y γ_pf frente a |α|² ∈ {1,...,5}, con el ajuste de pendiente de
    ...
    sin resonancia vestida: pendientes −1.28 a −1.62).
(b) La brecha de confinamiento, reportando para el modo elegido sus
    overlaps (P, n, a, a², σ_z) y su Im(λ) (frecuencia).

Comparar en ambos casos con el modelo efectivo **resonante** (sin el
término δ_1 a†a, con Δ_2₋=0 — igual que la variante "compensar" del
efectivo en la Tarea 22, que sí es válida).

**Entrega esperada**: tabla de la rejilla 9×9 (Tarea 24), ubicación
numérica de la resonancia vestida y comparación con (δ_1, 0), y las
tablas (a) y (b) de la Tarea 25 para los 4 valores de Γ₂/κ.
